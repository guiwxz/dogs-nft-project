...
const LexaPage: React.FC = () => {
  const { fetchDogsNft, postDogsNft, fetchCidImage, dogLexa, loading, deleteDogsNft } = useDogsNft();
  ...
  const handleSubmit = async (values: DogsNFTType) => {
    postDogsNft("Lexa", values, image.blob, () => setOpenModal(false));
  ...
  const handleRemove = ({ fileCid, _id }: DogsNFTType) => {
    if (fileCid && fileCid.length > 0) {
      deleteDogsNft(_id as string, "Lexa", fileCid);
    } else {
      alert("Cid nao encontrado!");
    }
  }
