...
const MellPage: React.FC = () => {
  const { fetchDogsNft, postDogsNft, fetchCidImage, dogLittle, loading, deleteDogsNft } = useDogsNft();
  ...
  const handleSubmit = async (values: DogsNFTType) => {
    postDogsNft("Little", values, image.blob, () => setOpenModal(false));
  ...
  const handleRemove = ({ fileCid, _id }: DogsNFTType) => {
    if (fileCid && fileCid.length > 0) {
      deleteDogsNft(_id as string, "Little", fileCid);
    } else {
      alert("Cid nao encontrado!");
    }
  }
...
*/
  return (
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <PageHeader onClick={() => setOpenModal(true)} title="Little" />
      </Grid>
      <Grid item xs={12}>
        <DataTable
          data={dogLittle}
